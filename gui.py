...
svm_tfidf_vectorizer = joblib.load(os.path.join(BASE_DIR, 'svm_tfidf_vectorizer.pkl'))
svm_model = joblib.load(os.path.join(BASE_DIR, 'svm_model.pkl'))
lr_tfidf_vectorizer = joblib.load(os.path.join(BASE_DIR, 'lr_tfidf_vectorizer.pkl'))
lr_model = joblib.load(os.path.join(BASE_DIR, 'lr_model.pkl'))
nb_tfidf_vectorizer = joblib.load(os.path.join(BASE_DIR, 'nb_tfidf_vectorizer.pkl'))
...
rf_tfidf_vectorizer = joblib.load(os.path.join(BASE_DIR, 'rf_tfidf_vectorizer.pkl'))
nn_vectorizer = joblib.load(os.path.join(BASE_DIR, 'nn_vectorizer.pkl'))
...
def process_text():
    ...
    #Algorithm 1: SVM
    svm_text_tfidf = svm_tfidf_vectorizer.transform([cleaned])
    svm_pred = svm_model.predict(svm_text_tfidf)[0]
    set_result(0, get_label(svm_pred))

    # Algorithm 2: Logistic Regression
    lr_text_tfidf = lr_tfidf_vectorizer.transform([cleaned])
    lr_pred = lr_model.predict(lr_text_tfidf)[0]
    set_result(1, get_label(lr_pred))

    # ── Algorithm 3: Naive Bayes ──
    nb_text_tfidf = nb_tfidf_vectorizer.transform([cleaned])
    nb_pred = nb_model.predict(nb_text_tfidf)[0]
    set_result(2, get_label(nb_pred))

    # ── Algorithm 4: Random Forest ──
    rf_text_tfidf = rf_tfidf_vectorizer.transform([cleaned])
    rf_pred = random_forest_model.predict(rf_text_tfidf)[0]
    set_result(3, get_label(rf_pred))

    # ── Algorithm 5: NN ──
    nn_text_tfidf = nn_vectorizer.transform([cleaned])
    nn_pred = nn_model.predict(nn_text_tfidf)[0]
    set_result(4, get_label(nn_pred))
