...
# ─── English Stop Words (no nltk dependency) ───
stop_words = {
...
# TODO: Load your other 4 models here
# Example:
...
def clean_text(text):
    """Pre-process text: remove commas, lowercase, remove stop words."""
    text = text.replace(',', '')
    text = text.lower()
    words = text.split()
...
def process_text():
    ...
    set_result(0, svm_pred)

    # ── Algorithm 2: (Add your model) ──
    # lr_pred = lr_model.predict(text_tfidf)[0]
    # set_result(1, lr_pred)
    set_result(1, "Model not loaded yet")

    # ── Algorithm 3: (Add your model) ──
    # nb_pred = nb_model.predict(text_tfidf)[0]
    # set_result(2, nb_pred)
    set_result(2, "Model not loaded yet")

    # ── Algorithm 4: (Add your model) ──
    # rf_pred = rf_model.predict(text_tfidf)[0]
    # set_result(3, rf_pred)
    set_result(3, "Model not loaded yet")

    # ── Algorithm 5: (Add your model) ──
    # knn_pred = knn_model.predict(text_tfidf)[0]
    # set_result(4, knn_pred)
    set_result(4, "Model not loaded yet")
